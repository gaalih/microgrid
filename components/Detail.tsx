...
function Detail({ data, label, header, className }: DetailProps) {
  return (
    ...
    >
      {header && (
        <h3 className="col-span-2 mb-1 text-center font-bold text-xs text-blue-custom">{header}</h3>
      )}
        ...
        <React.Fragment key={index}>
          <div>
            ...
            <h5 className="card-value text-xl font-bold">
              ...
              <span className="card-unit ml-1 text-xs">{label[key].unit ? Parse(label[key].unit.toString()) : label[key].unit}</span>
            </h5>
          </div>
        </React.Fragment>
      ))}
